...
kubectl -n logging rollout status statefulset/opensearch --timeout=180s || true

echo "⏳ Waiting for OpenSearch Dashboards to be ready..."
kubectl -n logging rollout status deployment/opensearch-dashboards --timeout=180s || true

echo "⏳ Waiting for Fluent Bit to be ready..."
kubectl -n logging rollout status daemonset/fluent-bit --timeout=180s || true

NODE_PORT=$(kubectl -n logging get svc opensearch-dashboards -o jsonpath='{.spec.ports[0].nodePort}')
echo "✅ Logging stack is up (or rolling out). OpenSearch Dashboards: http://localhost:${NODE_PORT}"
